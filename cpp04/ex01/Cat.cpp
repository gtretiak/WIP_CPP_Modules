...
Cat::Cat() : Animal("Cat"), myBrain_(new Brain()) {
	std::cout << "Cat was born" << std::endl;
}

Cat::Cat(const std::string name) : Animal(name), myBrain_(new Brain()) {
	std::cout << "Cat with name " << name << " was born" << std::endl;
}

Cat::Cat(const Cat &Another) : Animal(Another), myBrain_(new Brain(*Another.myBrain_)) {
