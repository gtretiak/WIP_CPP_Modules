...
Dog::Dog() : Animal("Dog"), myBrain_(new Brain()) {
	std::cout << "Dog was born" << std::endl;
}

Dog::Dog(const std::string name) : Animal(name), myBrain_(new Brain()) {
	std::cout << "Dog with name " << name << " was born" << std::endl;
}

Dog::Dog(const Dog &Another) : Animal(Another), myBrain_(new Brain(*Another.myBrain_)) {
